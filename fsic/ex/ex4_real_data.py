...
def job_nfsicJ10_stoopt(paired_source, tr, te, r, n_permute=None):
    ...
    with util.ContextTimer() as t:
        ...
        'reg': 1e-6, 'gwidthx_lb': medx2*1e-3, 'gwidthx_ub': medx2*1e3, 
        'gwidthy_lb': medy2*1e-3, 'gwidthy_ub': medy2*1e3}
...
def job_nfsicJ10_med(paired_source, tr, te, r, n_permute=None):
    ...
    return {'indtest': nfsic_med, 'test_result': nfsic_med_result, 'time_secs': t.secs}
...
def job_qhsic_med(paired_source, tr, te, r):
    ...
    if pdata.sample_size() >= 5000:
        ...
        fake_result = {'alpha': alpha, 'pvalue': 1, 'test_stat': -1 
                , 'h0_rejected': False, 'time_secs': 0, 'n_permute': n_permute}
...
class Ex4Job(IndependentJob):
    ...
    def __init__(self, aggregator, folder_path, prob_label, rep, job_func ):
        walltime = 60*59*24 
    ...
    def compute(self):
        ...
        paired_source, n, _ = exglo.get_problem_pickle(folder_path, prob_label)
...
from fsic.ex.ex4_real_data import job_nfsicJ10_stoopt
from fsic.ex.ex4_real_data import job_nfsicJ10_perm_stoopt
from fsic.ex.ex4_real_data import job_nfsicJ10_perm_med
from fsic.ex.ex4_real_data import job_nfsicJ10_med
from fsic.ex.ex4_real_data import job_qhsic_med
from fsic.ex.ex4_real_data import job_nyhsic_med
from fsic.ex.ex4_real_data import job_fhsic_med
from fsic.ex.ex4_real_data import job_rdcperm_med
from fsic.ex.ex4_real_data import Ex4Job
...
def run_problem(folder_path, prob_label):
    ...
    for r in range(reps):
        for mi, f in enumerate(method_job_funcs):
            ...
            if not is_rerun and glo.ex_file_exists(ex, prob_label, fname):
                ...
                job = Ex4Job(SingleResultAggregator(), folder_path, prob_label, r,
                        f)
    ...
    results = {'job_results': job_results, 'n': n, 'is_h0': is_h0,
